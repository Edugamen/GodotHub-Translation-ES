...
function AssetCard({
  ...
  return (
    <motion.div
      ...
      className="group relative flex flex-col overflow-hidden rounded-item bg-overlay border border-outline/50 hover:bg-raised hover:border-accent-dim/60 transition-colors"
    >
      <div className="relative h-24 shrink-0 bg-raised flex items-center justify-center overflow-hidden">
        {asset.icon_url && (
          <img
            src={asset.icon_url}
            alt=""
          ...
          {asset.support_level && (
            <span
              className={`shrink-0 inline-flex items-center px-2 py-0.5 rounded-tag text-[10px] font-semibold border ${
                SUPPORT_BADGE[asset.support_level] ?? 'bg-raised text-muted border-outline/50'
              }`}
            >
              {asset.support_level}
            </span>
...
export function AssetLibraryBrowser({ query }: { query: string }) {
  const { t } = useTranslation('common')
                  ...
                  <AssetCard
                    ...
                    actions={
                      <motion.button
                        ...
                        className={`focus-ring cursor-pointer flex items-center justify-center gap-1.5 w-full h-9 px-4 rounded-item text-xs font-semibold transition-all disabled:cursor-default ${
                          isInstalled
