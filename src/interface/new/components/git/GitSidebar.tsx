...
export function GitSidebar({ project, gitStatus, onClose, onRefresh }: Props) {
  ...
  const handleDiscardChanges = () =>
    doAction('discard', async () => {
      ...
      if (stashResult) {
        ...
        addToast('info', tc('git_stashed_to', { label: stashLabel }))
      }
  ...
  const handleStageFiles = async () => {
    ...
    try {
      ...
      setStagedFiles(new Set())
      await refreshAll()
      addToast('success', tc('git_files_staged', { count: stagedFiles.size }))
    } catch (e) {
  ...
  return (
    <div className="w-[400px] h-full flex flex-col overflow-hidden relative rounded-card bg-overlay border border-outline/50 shadow-2xl shadow-black/50">

      <div className="flex items-center justify-between px-5 py-4 border-b border-line shrink-0">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2.5">
            <span className="w-8 h-8 rounded-item bg-accent/10 border border-accent-dim/30 flex items-center justify-center shrink-0">
            ...
            <Tooltip content={tc('git_beta_tooltip')} side="bottom">
              <span className="text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-tag bg-amber/10 text-amber border border-amber/20 shrink-0">
                {tc('git_beta_badge')}
              </span>
            </Tooltip>
          </div>
          {isRepo && currentBranch && (
            <p className="text-[11px] font-mono text-muted mt-1.5 truncate">
              {currentBranch.name}
              {changedFiles.length > 0 && <span className="ml-1.5 text-amber">· {changedFiles.length} {tc('git_uncommitted')}</span>}
            </p>
          )}
        </div>
      ...
      {!isRepo ? (
        <div className="flex-1 flex flex-col items-center justify-center gap-4 p-8 text-center">
          <span className="w-16 h-16 rounded-tile bg-raised border border-outline/50 flex items-center justify-center">
            <IconGitBranch className="w-7 h-7 text-muted/40" />
          </span>
          <p className="text-sm text-muted">{tc('git_not_repo')}</p>
          <button disabled={busyAction === 'init'} onClick={handleInit}
            className="focus-ring cursor-pointer flex items-center gap-2 px-5 py-2.5 rounded-item bg-accent hover:bg-accent-bright disabled:opacity-50 disabled:cursor-not-allowed text-xs font-medium text-white transition-colors">
        ...
        <>
          ...
          <div className="flex items-center gap-1.5 px-4 py-3 border-b border-line shrink-0 flex-wrap">

            {mergeActive && !showMergeConflicts && (
              <div className="w-full flex items-center gap-2 px-3 py-2 rounded-item bg-danger/10 border border-danger/30 mb-1">
            ...
            <button disabled={busyAction !== null}
              ...
              className="focus-ring cursor-pointer flex items-center gap-1.5 px-3 py-2 rounded-item border border-danger/40 text-danger hover:bg-danger/10 hover:border-danger disabled:opacity-50 disabled:cursor-not-allowed text-[11px] font-medium transition-colors">
          ...
          >
            <div className="px-4 pb-6">
              ...
              <div className="px-2 pt-4 pb-2 border-b border-line">
                <div className="flex items-center justify-between mb-2">
                  <h4 className="text-[10px] font-semibold uppercase tracking-wider text-muted/50">{t('remote_title')}</h4>
                ...
                {showRemoteInput ? (
                  <div className="flex items-center gap-1.5 mb-1">
                    <input type="text" value={remoteInput} onChange={(e) => setRemoteInput(e.target.value)}
                      placeholder={remoteUrl ? t('new_remote_url') : tc('git_remote_placeholder')}
                      onKeyDown={(e) => { if (e.key === 'Enter') handleSetRemote(); if (e.key === 'Escape') setShowRemoteInput(false) }}
                      className="flex-1 focus-ring bg-base border border-outline/50 rounded-item px-2.5 py-1.5 text-xs text-ink placeholder:text-muted transition-colors focus:border-accent-dim outline-none" autoFocus />
                    <button onClick={handleSetRemote} disabled={busyAction !== null || !remoteInput.trim()}
                      className="focus-ring cursor-pointer p-1.5 rounded-item bg-accent hover:bg-accent-bright disabled:opacity-40 disabled:cursor-not-allowed text-white transition-colors">
                ...
                <div className="px-2 pt-4 pb-2 border-b border-line">
                  <div className="flex items-center justify-between mb-2">
                    <h4 className="text-[10px] font-semibold uppercase tracking-wider text-muted/50">{t('branches_title')}</h4>
                    <button onClick={() => setShowCreateBranch((v) => !v)}
                      className="focus-ring cursor-pointer text-[10px] text-accent-bright hover:underline transition-colors">{t('new_branch_btn')}</button>
                  </div>
                  {showCreateBranch && (
                    <div className="flex items-center gap-1.5 mb-2">
                      <input type="text" value={newBranchName} onChange={(e) => setNewBranchName(e.target.value)}
                        placeholder={tc('git_branch_name_placeholder')}
                  ...
                  {branchesLoading ? (
                    ...
                    <p className="text-[11px] text-muted/50 py-2">No branches found.</p>
                  ) : (
                    <div className="flex flex-col gap-0.5 max-h-[150px] overflow-y-auto new-ui-scroll-viewport">
                      {branches.map((b) => (
                        <div key={b.name}
                          className={`group flex items-center gap-2 px-2.5 py-1.5 rounded-item transition-colors ${b.is_current ? 'bg-accent/10 text-ink' : 'hover:bg-raised text-muted'}`}>
                ...
                <div className="px-2 pt-4 pb-2 border-b border-line">
                  <div className="flex items-center justify-between mb-2">
                    <h4 className="text-[10px] font-semibold uppercase tracking-wider text-muted/50">Changes</h4>
                  ...
                  {changesLoading ? (
                    ...
                    <p className="text-[11px] text-muted/50 py-2">{t('working_tree_clean')}</p>
                  ) : (
                    <div className="flex flex-col gap-0.5 max-h-[220px] overflow-y-auto new-ui-scroll-viewport">
                          ...
                          {gitStagedFiles.map((f, i) => {
                            ...
                            return (
                              <div key={`git-staged-${f.path}-${i}`}
                                className="flex items-center gap-2 px-2.5 py-1.5 rounded-item bg-mint/5 hover:bg-mint/10 transition-colors">
                          ...
                          {pendingStageFiles.map((f, i) => {
                            ...
                            return (
                              <div key={`staged-${f.path}-${i}`}
                                className="flex items-center gap-2 px-2.5 py-1.5 rounded-item bg-accent/5 hover:bg-accent/10 transition-colors">
                          ...
                          {unstagedFiles.map((f, i) => {
                            ...
                            return (
                              <div key={`unstaged-${f.path}-${i}`}
                                className="flex items-center gap-2 px-2.5 py-1.5 rounded-item hover:bg-raised transition-colors">
                  ...
                  {changedFiles.length > 0 && (
                    <div className="mt-3 flex flex-col gap-2 pb-1">
                      <textarea
                        ...
                        className="focus-ring w-full bg-base border border-outline/50 rounded-item px-3 py-2 text-xs text-ink placeholder:text-muted transition-colors focus:border-accent-dim outline-none resize-none"
                      />
                      <div className="flex items-center gap-3">
                        <Checkbox checked={amendMode} onChange={(v) => setAmendMode(v)}>{t('amend')}</Checkbox>
                        {remoteUrl && (
                          <Checkbox checked={pushAfterCommit} onChange={(v) => setPushAfterCommit(v)}>{t('push_after')}</Checkbox>
                        )}
                        <div className="flex-1" />
                        <span className="text-[10px] text-muted/50">Ctrl+Enter</span>
                        <button
                          disabled={busyAction !== null || !commitMessage.trim() || changedFiles.length === 0}
                          onClick={handleCommit}
                          className="focus-ring cursor-pointer flex items-center gap-1.5 px-4 py-2 rounded-item bg-accent hover:bg-accent-bright disabled:opacity-40 disabled:cursor-not-allowed text-xs font-medium text-white transition-colors"
                        >
                          <IconCheck className="w-3 h-3" />
                          {busyAction === 'commit'
                ...
                {(undoHistory.length > 0 || redoHistory.length > 0) && (
                  <div className="px-2 pt-4 pb-3 border-b border-line">
                    <div className="flex items-center justify-between mb-2">
                      <h4 className="text-[10px] font-semibold uppercase tracking-wider text-muted/50">Actions</h4>
                    </div>
                    <div className="flex flex-col gap-0.5">
                      {redoHistory.slice(0, 3).map((entry) => (
                        <button key={`redo-${entry.id}`}
                          ...
                          disabled={busyAction !== null}
                          className="flex items-center gap-2 px-2.5 py-1.5 rounded-item opacity-40 hover:opacity-100 hover:bg-raised transition-all disabled:opacity-20 disabled:cursor-not-allowed w-full text-left cursor-pointer">
                      ...
                      {undoHistory.slice(0, 5).map((entry) => (
                        <button key={`undo-${entry.id}`}
                          ...
                          disabled={busyAction !== null}
                          className="flex items-center gap-2 px-2.5 py-1.5 rounded-item hover:bg-raised transition-all disabled:opacity-40 disabled:cursor-not-allowed w-full text-left cursor-pointer group">
                ...
                <div className="px-2 pt-4 pb-2 border-b border-line">
                  <div className="flex items-center justify-between mb-2">
                    <h4 className="text-[10px] font-semibold uppercase tracking-wider text-muted/50">Stashes</h4>
                  </div>
                  {stashesLoading ? (
                    ...
                    <p className="text-[11px] text-muted/50 py-2">No stashes.</p>
                  ) : (
                    <div className="flex flex-col gap-0.5 max-h-[120px] overflow-y-auto new-ui-scroll-viewport">
                      {stashes.map((s) => (
                        <div key={s.index} className="group flex items-center gap-2 px-2.5 py-1.5 rounded-item hover:bg-raised transition-colors">
                          <IconHistory className="w-3 h-3 text-muted shrink-0" />
                ...
                <div className="px-2 pt-4 pb-2">
                  <h4 className="text-[10px] font-semibold uppercase tracking-wider text-muted/50 mb-3">{t('recent_commits')}</h4>
                  {logLoading ? (
                    <div className="flex items-center justify-center py-6"><IconRefresh className="w-4 h-4 animate-spin text-muted" /></div>
                  ) : logEntries.length === 0 ? (
                    <div className="border border-dashed border-outline/50 rounded-item py-6 text-center"><p className="text-xs text-muted">{t('no_commits_found')}</p></div>
                  ) : (
                    <div className="flex flex-col gap-1.5">
                      {logEntries.map((entry, i) => (
                        <div key={`${entry.hash}-${i}`}
                          onClick={() => { if (remoteUrl) { const u = `${remoteUrl.replace(/\/+$/, '')}/commit/${entry.hash}`; window.open(u, '_blank') } }}
                          className={`group flex items-start gap-3 px-3 py-2 rounded-item transition-colors ${remoteUrl ? 'cursor-pointer hover:bg-raised' : 'cursor-default'}`}>
        ...
        <AnimatePresence>
          {toasts.map((toast) => (
            <motion.div
              ...
              className={`pointer-events-auto flex items-center gap-2.5 px-3.5 py-2.5 rounded-item shadow-lg border text-xs max-w-full ${
                toast.type === 'success'
      ...
      <AnimatePresence>
        {showStashPushConfirm && (
          <ConfirmDialog
            title={tc('git_stash_title')}
            description={tc('git_stash_desc')}
            confirmLabel={tc('git_stash_confirm')}
      ...
      <AnimatePresence>
        {showPushConfirm && (
          <ConfirmDialog
            title={tc('git_push_uncommitted_title')}
            description={tc('git_push_uncommitted_desc')}
            confirmLabel={tc('git_push_uncommitted_confirm')}
      ...
      <AnimatePresence>
        {showForcePushConfirm && (
          <ConfirmDialog
            title={tc('git_force_push_title')}
            description={tc('git_force_push_desc')}
            confirmLabel={tc('git_force_push_confirm')}
            variant="danger"
